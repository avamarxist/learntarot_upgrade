...
for card in scrape_df['Card ID']:
    if card[0] == 't':
        ...
        if len(tags_u) == 1 or card == 'p6':
            ...
            opposing.append(str(tags_u[0]))        #changed these indices to 0 and 1 bc keywords is no longer in the list; could be wrong
            reinforcing.append(str(tags_u[1]))
        ...
        for i in range(len(tags_dt)):
            actID += 1
            dt = tags_dt[i].replace('<b>','')
            actions.append([actID,card,dt])
            ...
            for sact in subacts:
                sactID += 1
                subactions.append([sactID, actID, sact])
...
scrape_cols = ['Card ID','Title','Suit','Links','Small Image','Opposing','Reinforcing','Previous']
...
actions_df.columns = ['act_id','root_card','act_text']
subactions_df = pd.DataFrame(subactions)
subactions_df.columns = ['sact_id', 'act_id','sact_text']
...
actions_df.to_csv(path_or_buf = 'db/actions.csv', sep = '|', index = False)
subactions_df.to_csv(path_or_buf = 'db/subactions.csv', sep = '|', index = False)
